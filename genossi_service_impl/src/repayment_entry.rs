...
impl<Deps: RepaymentEntryServiceDeps> RepaymentEntryService for RepaymentEntryServiceImpl<Deps> {
    ...
    async fn batch_toggle_status(
        ...
        for (idx, entry_id) in input.entry_ids.iter().enumerate() {
            ...
            // CR-01 Fix (WR-01 same root cause): Re-read after audited_update!
            // to get the new version UUID generated by the DAO. Without this refresh the
            // returned Vec carries N stale versions (one per entry). Re-Read
            // runs in the same transaction — soft-delete in the same Tx is
            // impossible (single-writer per service method), so `None` here
            // is an internal consistency error, not a user-facing race.
            //
            // BL-01 Fix: Map this `None` branch to InternalError → HTTP 500,
            // NOT EntityNotFound → 404. A 404 here would collide with the
            // CR-02 batch-error 404 ("one of the requested ids is unknown")
            // — two semantically different 404s on the same endpoint that
            // the client cannot distinguish. The DAO inconsistency must
            // surface as a 500 so monitoring catches it.
            let refreshed = self
                .repayment_entry_dao
                .find_by_id(*entry_id, tx.clone())
                .await?
                .ok_or_else(|| {
                    ServiceError::InternalError(Arc::from(format!(
                        "Re-Read after audited_update! returned None for RepaymentEntry {} \
                         in batch_toggle_status — internal consistency error \
...
mod tests {
    ...
    async fn test_update_entry_status_open_to_contacted_succeeds() {
        ...
        // CR-01 (Phase 08 gap-closure): update_repayment_entry calls find_by_id
        // three times now: (1) pre-update Edit-Matrix load, (2) audited_update!
        // internal load, (3) Re-Read after update for fresh version. The first
        // two return the pre-update entity; the third returns the post-update
        // entity (new status, new version).
        let entity = entry_in_status(
            Uuid::new_v4(),
            Uuid::new_v4(),
            RepaymentEntryStatus::Open,
    ...
    async fn test_update_entry_status_contacted_to_open_succeeds() {
        ...
        // CR-01 (Phase 08 gap-closure): Re-Read returns post-update entity with
        // new status (Open) and new version.
        let entity = entry_in_status(
            Uuid::new_v4(),
            Uuid::new_v4(),
    ...
    async fn test_batch_toggle_success() {
        ...
        // CR-01 (Phase 08 gap-closure): per iteration the loop performs four
        // mock interactions in strict order: (1) pre-update load -> Open,
        // (2) audited_update! internal load -> Open, (3) DAO.update, (4) Re-Read
        // post-update -> Contacted. 12 interactions total for 3 entries.
        let phase_id = Uuid::new_v4();
        let entry1 = entry_in_status(Uuid::new_v4(), phase_id, RepaymentEntryStatus::Open, 1);
        ...
        let post1 = RepaymentEntryEntity {
            status: RepaymentEntryStatus::Contacted,
            version: Uuid::new_v4(),
            ..entry1.clone()
        };
        let post2 = RepaymentEntryEntity {
            status: RepaymentEntryStatus::Contacted,
            version: Uuid::new_v4(),
            ..entry2.clone()
        };
        let post3 = RepaymentEntryEntity {
            status: RepaymentEntryStatus::Contacted,
            version: Uuid::new_v4(),
            ..entry3.clone()
        };
        ...
        for (pre, post) in [(&entry1, &post1), (&entry2, &post2), (&entry3, &post3)] {
            let pre_c1 = pre.clone();
            entry_dao
                .expect_find_by_id()
                .times(1)
                .in_sequence(&mut seq)
                .returning(move |_, _| Ok(Some(pre_c1.clone())));
            let pre_c2 = pre.clone();
            entry_dao
                .expect_find_by_id()
                .times(1)
                .in_sequence(&mut seq)
                .returning(move |_, _| Ok(Some(pre_c2.clone())));
            entry_dao
                .expect_update()
                .times(1)
                .in_sequence(&mut seq)
            ...
            entry_dao
                ...
                .times(1)
                .in_sequence(&mut seq)
                .returning(move |_, _| Ok(Some(post_c.clone())));
        }
    ...
    async fn test_update_repayment_entry_rereads_after_audited_update_returns_new_version() {
        ...
        // Re-Read returns the post-update DB snapshot: new status (Contacted,
