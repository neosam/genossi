...
macro_rules! gen_service_impl {
    ...
    (@generate_trait $dependencies:ident, $($field_name:ident : $field_type:path),*) => {
        pub trait $dependencies {
            type Context: Send + Sync + Clone + Eq + std::fmt::Debug + genossi_service::claim_context::ClaimContext + 'static;
            type Transaction: genossi_dao::Transaction + Send + Sync + Clone + std::fmt::Debug + 'static;
