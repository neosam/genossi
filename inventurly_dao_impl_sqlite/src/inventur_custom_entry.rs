...
use inventurly_dao::{
    inventur_custom_entry::{InventurCustomEntryDao, InventurCustomEntryEntity},
    DaoError,
};
use sqlx::SqlitePool;
use std::sync::Arc;
...
impl TryFrom<&InventurCustomEntryDb> for InventurCustomEntryEntity {
    type Error = DaoError;

    fn try_from(db: &InventurCustomEntryDb) -> Result<Self, Self::Error> {
        ...
        Ok(InventurCustomEntryEntity {
            id: Uuid::from_slice(&db.id)?,
            inventur_id: Uuid::from_slice(&db.inventur_id)?,
            custom_product_name: Arc::from(db.custom_product_name.as_str()),
            rack_id: db
                .rack_id
                .as_ref()
            ...
            ean: db.ean.as_ref().map(|e| Arc::from(e.as_str())),
...
pub struct InventurCustomEntryDaoImpl {
    pub pool: Arc<SqlitePool>,
}

impl InventurCustomEntryDaoImpl {
...
#[async_trait]
impl InventurCustomEntryDao for InventurCustomEntryDaoImpl {
    ...
    async fn dump_all(
        ...
        let rows = sqlx::query_as::<_, InventurCustomEntryDb>(
            "SELECT id, inventur_id, custom_product_name, rack_id, container_id, count, weight_grams,
                    measured_by, measured_at, notes, ean, review_state, created, deleted, version
             FROM inventur_custom_entry ORDER BY measured_at DESC"
        )
        ...
        rows.iter()
            .map(InventurCustomEntryEntity::try_from)
    ...
    async fn create(
        ...
        let custom_product_name = entity.custom_product_name.to_string();
        ...
        let ean = entity.ean.as_ref().map(|e| e.to_string());
        let review_state = entity.review_state.to_string();

        sqlx::query(
            "INSERT INTO inventur_custom_entry
             (id, inventur_id, custom_product_name, rack_id, container_id, count, weight_grams,
              measured_by, measured_at, notes, ean, review_state, created, version)
             VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)"
        )
        .bind(id)
        .bind(inventur_id)
        .bind(custom_product_name)
        .bind(rack_id)
        .bind(container_id)
        .bind(entity.count)
    ...
    async fn update(
        ...
        let exists = sqlx::query_scalar::<_, i32>(
            "SELECT COUNT(*) FROM inventur_custom_entry WHERE id = ? AND deleted IS NULL",
        )
        .bind(id.clone())
        ...
        let rows_affected = sqlx::query(
            "UPDATE inventur_custom_entry
             SET inventur_id = ?, custom_product_name = ?, rack_id = ?, container_id = ?,
                 count = ?, weight_grams = ?, measured_by = ?, measured_at = ?,
                 notes = ?, ean = ?, review_state = ?, deleted = ?, version = ?
