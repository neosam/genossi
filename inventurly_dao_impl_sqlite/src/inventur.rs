use async_trait::async_trait;
use inventurly_dao::{
    inventur::{InventurDao, InventurEntity},
    DaoError,
};
use sqlx::SqlitePool;
use std::sync::Arc;
use time::PrimitiveDateTime;
use uuid::Uuid;

use crate::TransactionImpl;

#[derive(Debug, sqlx::FromRow)]
struct InventurDb {
    id: Vec<u8>,
    name: String,
    description: String,
    start_date: String,
    end_date: Option<String>,
    status: String,
    created_by: String,
    created: String,
    deleted: Option<String>,
    version: Vec<u8>,
    token: Option<String>,
}

impl TryFrom<&InventurDb> for InventurEntity {
    type Error = DaoError;

    fn try_from(db: &InventurDb) -> Result<Self, Self::Error> {
        // Try multiple datetime formats to handle different storage formats
        fn parse_datetime(s: &str) -> Result<PrimitiveDateTime, time::error::Parse> {
            // First try ISO8601 format (what we should be using)
            if let Ok(dt) =
                PrimitiveDateTime::parse(s, &time::format_description::well_known::Iso8601::DEFAULT)
            {
                return Ok(dt);
            }

            // Then try SQLite default format with microseconds
            let sqlite_format = time::format_description::parse(
                "[year]-[month]-[day] [hour]:[minute]:[second].[subsecond]",
            )
            .unwrap(); // This format should always parse correctly
            if let Ok(dt) = PrimitiveDateTime::parse(s, &sqlite_format) {
                return Ok(dt);
            }

            // Try SQLite format without microseconds
            let sqlite_simple =
                time::format_description::parse("[year]-[month]-[day] [hour]:[minute]:[second]")
                    .unwrap(); // This format should always parse correctly
            PrimitiveDateTime::parse(s, &sqlite_simple)
        }

        Ok(InventurEntity {
            id: Uuid::from_slice(&db.id)?,
            name: Arc::from(db.name.as_str()),
            description: Arc::from(db.description.as_str()),
            start_date: parse_datetime(&db.start_date)?,
            end_date: db
                .end_date
                .as_ref()
                .map(|d| parse_datetime(d))
                .transpose()?,
            status: Arc::from(db.status.as_str()),
            created_by: Arc::from(db.created_by.as_str()),
            created: parse_datetime(&db.created)?,
            deleted: db.deleted.as_ref().map(|d| parse_datetime(d)).transpose()?,
            version: Uuid::from_slice(&db.version)?,
            token: db.token.as_ref().map(|t| Arc::from(t.as_str())),
        })
    }
}

pub struct InventurDaoImpl {
    pub pool: Arc<SqlitePool>,
}

impl InventurDaoImpl {
    pub fn new(pool: Arc<SqlitePool>) -> Self {
        Self { pool }
    }
}

#[async_trait]
impl InventurDao for InventurDaoImpl {
    type Transaction = TransactionImpl;

    async fn dump_all(&self, tx: Self::Transaction) -> Result<Arc<[InventurEntity]>, DaoError> {
        let rows = sqlx::query_as::<_, InventurDb>(
            "SELECT id, name, description, start_date, end_date, status, created_by, created, deleted, version, token
             FROM inventur ORDER BY start_date DESC"
        )
        .fetch_all(tx.tx.lock().await.as_mut())
        .await
        .map_err(|e| DaoError::DatabaseError(Arc::from(e.to_string())))?;

        rows.iter()
            .map(InventurEntity::try_from)
            .collect::<Result<Vec<_>, _>>()
            .map(|v| v.into())
    }

    async fn create(
        &self,
        entity: &InventurEntity,
        _process: &str,
        tx: Self::Transaction,
    ) -> Result<(), DaoError> {
        let id = entity.id.as_bytes().to_vec();
        let version = entity.version.as_bytes().to_vec();
        let format = &time::format_description::well_known::Iso8601::DEFAULT;
        let created = entity
            .created
            .assume_utc()
            .format(format)
            .map_err(|e| DaoError::ParseError(Arc::from(e.to_string())))?;
        let start_date = entity
            .start_date
            .assume_utc()
            .format(format)
            .map_err(|e| DaoError::ParseError(Arc::from(e.to_string())))?;
        let end_date = match entity.end_date {
            Some(dt) => Some(
                dt.assume_utc()
                    .format(format)
                    .map_err(|e| DaoError::ParseError(Arc::from(e.to_string())))?,
            ),
            None => None,
        };
        let name = entity.name.to_string();
        let description = entity.description.to_string();
        let status = entity.status.to_string();
        let created_by = entity.created_by.to_string();
        let token = entity.token.as_ref().map(|t| t.to_string());

        sqlx::query(
            "INSERT INTO inventur (id, name, description, start_date, end_date, status, created_by, created, version, token)
             VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)"
        )
        .bind(id)
        .bind(name)
        .bind(description)
        .bind(start_date)
        .bind(end_date)
        .bind(status)
        .bind(created_by)
        .bind(created)
        .bind(version)
        .bind(token)
        .execute(tx.tx.lock().await.as_mut())
        .await
        .map_err(|e| DaoError::DatabaseError(Arc::from(e.to_string())))?;

        Ok(())
    }

    async fn update(
        &self,
        entity: &InventurEntity,
        _process: &str,
        tx: Self::Transaction,
    ) -> Result<(), DaoError> {
        let id = entity.id.as_bytes().to_vec();
        let old_version = entity.version.as_bytes().to_vec();
        let new_version = Uuid::new_v4().as_bytes().to_vec();
        let format = &time::format_description::well_known::Iso8601::DEFAULT;
        let start_date = entity
            .start_date
            .assume_utc()
            .format(format)
            .map_err(|e| DaoError::ParseError(Arc::from(e.to_string())))?;
        let end_date = match entity.end_date {
            Some(dt) => Some(
                dt.assume_utc()
                    .format(format)
                    .map_err(|e| DaoError::ParseError(Arc::from(e.to_string())))?,
            ),
            None => None,
        };
        let name = entity.name.to_string();
        let description = entity.description.to_string();
        let status = entity.status.to_string();
        let created_by = entity.created_by.to_string();
        let token = entity.token.as_ref().map(|t| t.to_string());

        // Format deleted timestamp if present
        let deleted = match entity.deleted {
            Some(dt) => Some(
                dt.assume_utc()
                    .format(format)
                    .map_err(|e| DaoError::ParseError(Arc::from(e.to_string())))?,
            ),
            None => None,
        };

        // First check if the entity exists
        let exists = sqlx::query_scalar::<_, i32>(
            "SELECT COUNT(*) FROM inventur WHERE id = ? AND deleted IS NULL",
        )
        .bind(id.clone())
        .fetch_one(tx.tx.lock().await.as_mut())
        .await
        .map_err(|e| DaoError::DatabaseError(Arc::from(e.to_string())))?;

        if exists == 0 {
            return Err(DaoError::NotFound);
        }

        let rows_affected = sqlx::query(
            "UPDATE inventur
             SET name = ?, description = ?, start_date = ?, end_date = ?, status = ?,
                 created_by = ?, deleted = ?, version = ?, token = ?
             WHERE id = ? AND version = ? AND deleted IS NULL",
        )
        .bind(name)
        .bind(description)
        .bind(start_date)
        .bind(end_date)
        .bind(status)
        .bind(created_by)
        .bind(deleted)
        .bind(new_version)
        .bind(token)
        .bind(id)
        .bind(old_version)
        .execute(tx.tx.lock().await.as_mut())
        .await
        .map_err(|e| DaoError::DatabaseError(Arc::from(e.to_string())))?;

        if rows_affected.rows_affected() == 0 {
            return Err(DaoError::ConflictError(Arc::from(
                "Version mismatch - entity was modified by another transaction",
            )));
        }

        Ok(())
    }
}
