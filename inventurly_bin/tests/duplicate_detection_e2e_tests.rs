...
impl TestApp {
    ...
    async fn create_test_product(&self, ean: &str, name: &str, sales_unit: &str, requires_weighing: bool) -> String {
        let product = json!({
            "ean": ean,
            "name": name,
            "short_name": name,
        ...
        assert!(response.status().is_success(), "Failed to create product: {}", response.status());
        
        let created_product: serde_json::Value = response.json().await.expect("Failed to parse response");
    ...
    async fn get_duplicate_detection_by_ean(&self, ean: &str) -> serde_json::Value {
        let response = self.client
            .get(&format!("{}/duplicate-detection/products/{}", self.base_url, ean))
            .send()
            .await
            .expect("Failed to send request");
    ...
    async fn check_potential_duplicate(&self, name: &str, sales_unit: &str, requires_weighing: bool) -> serde_json::Value {
...
async fn test_duplicate_detection_find_word_order_duplicates() {
    ...
    let _ean1 = app.create_test_product("1001", "Macadamia süß salzig", "130g", false).await;
    let ean2 = app.create_test_product("1002", "süß salzig Macadamia", "130g", false).await;
    ...
    for duplicate_result in duplicates_array {
        ...
        assert!(!matches.is_empty(), "Should find at least one duplicate match");
        ...
        assert!(score > 0.5, "Similarity score should be > 0.5 for word order variations, got {}", score);
    }
    ...
    assert!(!matches.is_empty(), "Should find duplicates for specific product");
...
async fn test_duplicate_detection_typo_detection() {
    ...
    let _ean1 = app.create_test_product("2001", "Macadamia süß", "100g", false).await;
    let ean2 = app.create_test_product("2002", "Macadamia süss", "100g", false).await; // ü vs ss
    ...
    if !matches.is_empty() {
        let first_match = &matches[0];
        let score = first_match["similarity_score"].as_f64().unwrap();
        assert!(score > 0.4, "Should have reasonable similarity for typos, got {}", score);
        ...
        assert!(levenshtein > 0.5, "Levenshtein should detect typo similarity");
    } else {
...
async fn test_duplicate_detection_category_aware() {
    ...
    let _ean1 = app.create_test_product("3001", "Test Product", "100g", false).await;
    let ean2 = app.create_test_product("3002", "Test Product", "1kg", true).await; // Different unit and weighing
...
async fn test_check_potential_duplicate_api() {
    ...
    let _ean1 = app.create_test_product("4001", "Bio Apfelsaft naturtrüb", "1L", false).await;
    let _ean2 = app.create_test_product("4002", "Olivenöl Extra Vergine", "500ml", false).await;

    // Check for potential duplicate
    let matches = app.check_potential_duplicate("Apfelsaft Bio naturtrüb", "1L", false).await;
    let matches_array = matches.as_array().unwrap();
    
    assert!(!matches_array.is_empty(), "Should find potential duplicates");
    
    let first_match = &matches_array[0];
    let product_name = first_match["product"]["name"].as_str().unwrap();
    assert!(product_name.contains("Apfelsaft"), "Should match similar product");
...
async fn test_check_potential_duplicate_no_matches() {
    ...
    // Create products
    let _ean1 = app.create_test_product("5001", "Macadamia Nüsse", "130g", false).await;

    // Check for completely different product
    let matches = app.check_potential_duplicate("Totally Different Product", "2kg", true).await;
    let matches_array = matches.as_array().unwrap();
    
    // Should find no matches due to completely different name
    assert!(matches_array.is_empty(), "Should find no matches for completely different product");
}
...
async fn test_duplicate_detection_with_custom_config() {
    ...
    let _ean1 = app.create_test_product("6001", "Test Product One", "100g", false).await;
    let ean2 = app.create_test_product("6002", "Test Product Two", "100g", false).await;

    // Test with custom configuration (lower threshold)
    let response = app.client
        .get(&format!("{}/duplicate-detection/products/{}?similarity_threshold=0.3&word_order_weight=0.5", 
                     app.base_url, ean2))
        .send()
        .await
        .expect("Failed to send request");
    ...
    let duplicates: serde_json::Value = response.json().await.expect("Failed to parse response");
...
async fn test_duplicate_detection_real_world_german_examples() {
    ...
    for duplicate_result in duplicates_array {
        ...
        for match_item in matches {
            ...
            if score > 0.5 || confidence == "Medium" || confidence == "High" || confidence == "VeryHigh" {
                found_reasonable_matches = true;
                break;
            }
        }
        if found_reasonable_matches { break; }
    }
    ...
    assert!(found_reasonable_matches, "Should find at least some reasonable similarity matches");
}
...
async fn test_duplicate_detection_error_cases() {
    ...
    let response = app.client
        .get(&format!("{}/duplicate-detection/products/nonexistent", app.base_url))
        .send()
        .await
        .expect("Failed to send request");

    assert_eq!(response.status(), 404, "Should return 404 for non-existent product");
