...
mod tests {
    ...
    use inventurly_rest_types::{Price, ProductTO};
    use reqwest::Client;
    use serde_json::json;
    use sqlx::SqlitePool;
    use std::sync::Arc;
    ...
    impl TestContext {
        ...
        async fn update_product_with_version(
            &self,
            ean: &str,
            product: &ProductTO,
            name: &str,
    ...
    async fn test_e2e_create_product() {
        ...
        let product = ctx
            .create_product("4260474470041", "Test Product", 599)
            .await;
    ...
    async fn test_e2e_update_product() {
        ...
        // Create a product
        let created = ctx
            .create_product("4260474470041", "Original Name", 599)
            .await;

        // Update the product
        let updated = ctx
            .update_product_with_version("4260474470041", &created, "Updated Name", 799)
            .await;
    ...
    async fn test_e2e_duplicate_ean() {
        ...
        ctx.create_product("4260474470041", "First Product", 599)
            .await;
        ...
        assert_eq!(
            response.status(),
            400,
            "Should fail with 400 for duplicate EAN"
        );
