    ...
    pub fn calculate_similarity(
        ...
        let final_score = if config.category_aware {
            ...
            (exact_match * config.exact_match_weight +
             ...
             category_score * category_weight) / adjusted_total_weight
        } else {
            exact_match * config.exact_match_weight +
            word_order * config.word_order_weight +
            levenshtein * config.levenshtein_weight +
...
mod tests {
    ...
    fn test_text_normalizer_normalize() {
        assert_eq!(TextNormalizer::normalize("Macadamia Süß Salzig"), "macadamia süß salzig");
        assert_eq!(TextNormalizer::normalize("  MULTIPLE   SPACES  "), "multiple spaces");
        assert_eq!(TextNormalizer::normalize("With-Special!@#Characters"), "withspecialcharacters");
    ...
    fn test_exact_match_similarity() {
        ...
        assert_eq!(SimilarityCalculator::exact_match("Macadamia Süß", "macadamia süß"), 1.0);
        assert_eq!(SimilarityCalculator::exact_match("TEST", "test"), 1.0);
        
        // Different
        assert_eq!(SimilarityCalculator::exact_match("Macadamia", "Cashew"), 0.0);
    ...
    fn test_word_order_similarity() {
        ...
        let score = SimilarityCalculator::word_order_similarity("Macadamia süß salzig", "süß salzig Macadamia");
        println!("Word order score for same words: {}", score);
        assert_eq!(score, 1.0); // All words match, just reordered
        
        // Partial word overlap
        let score = SimilarityCalculator::word_order_similarity("Macadamia süß", "Macadamia salzig");
    ...
    fn test_levenshtein_similarity() {
        // Identical strings
        assert_eq!(SimilarityCalculator::levenshtein_similarity("test", "test"), 1.0);
    ...
    fn test_jaro_winkler_similarity() {
        // Identical strings
        assert_eq!(SimilarityCalculator::jaro_winkler_similarity("test", "test"), 1.0);
    ...
    fn test_category_similarity() {
        ...
        assert_eq!(SimilarityCalculator::category_similarity(&product1, &product2), 1.0);
    ...
    fn test_calculate_similarity_german_examples() {
        ...
        let (score, algorithms) = SimilarityCalculator::calculate_similarity(&product1, &product2, &config);
        ...
        let (score, algorithms) = SimilarityCalculator::calculate_similarity(&product1, &product3, &config);
    ...
    fn test_calculate_similarity_category_aware() {
        ...
        let (score_aware, _) = SimilarityCalculator::calculate_similarity(&product1, &product2, &config);
        
        // Without category awareness
        config.category_aware = false;
        let (score_not_aware, _) = SimilarityCalculator::calculate_similarity(&product1, &product2, &config);
    ...
    fn test_match_confidence_levels() {
        assert!(matches!(MatchConfidence::from_score(0.97), MatchConfidence::VeryHigh));
        assert!(matches!(MatchConfidence::from_score(0.90), MatchConfidence::High));
        assert!(matches!(MatchConfidence::from_score(0.75), MatchConfidence::Medium));
        assert!(matches!(MatchConfidence::from_score(0.65), MatchConfidence::Low));
    }
    ...
    fn test_real_world_german_product_examples() {
        ...
        let examples = vec![
            ("Macadamia Nüsse süß salzig 130g", "süß salzig Macadamia Nüsse 130g"),
