use crate::i18n::Key;
use rest_types::{MemberStatusTO, MemberTO, SalutationTO};
...
pub struct ColumnDef {
    ...
    pub select_options: &'static [SelectOption],
}

fn noop_set(_m: &mut MemberTO, _v: &str) {}

fn render_salutation(m: &MemberTO, _i18n: &crate::i18n::I18n) -> String {
    m.salutation
        .as_ref()
        .map(|s| s.as_str().to_string())
        .unwrap_or_default()
}
fn render_title(m: &MemberTO, _i18n: &crate::i18n::I18n) -> String {
...
fn render_status(m: &MemberTO, i18n: &crate::i18n::I18n) -> String {
    match m.status {
        MemberStatusTO::Normal => i18n.t(Key::MemberStatusNormal).to_string(),
        MemberStatusTO::FehlerhaftErfasst => i18n.t(Key::MemberStatusFehlerhaftErfasst).to_string(),
    }
}
...
fn set_opt(v: &str) -> Option<String> {
    if v.is_empty() {
        None
    } else {
        Some(v.to_string())
...
static SALUTATION_OPTIONS: &[SelectOption] = &[
    SelectOption {
        value: "Herr",
        label: "Herr",
    },
...
static STATUS_OPTIONS: &[SelectOption] = &[
    SelectOption {
        value: "Normal",
        label: "Normal",
    },
    SelectOption {
        value: "FehlerhaftErfasst",
        label: "Fehlerhaft erfasst",
    },
];
...
pub static ALL_COLUMNS: &[ColumnDef] = &[
    ...
    ColumnDef {
        key: "last_name",
        label_key: Key::LastName,
        editable: true,
        input_type: InputType::Text,
        render: render_last_name,
        get_value: |m| m.last_name.clone(),
        set_value: |m, v| {
            m.last_name = v.to_string();
        },
        select_options: &[],
    },
    ColumnDef {
        key: "first_name",
        label_key: Key::FirstName,
        editable: true,
        input_type: InputType::Text,
        render: render_first_name,
        get_value: |m| m.first_name.clone(),
        set_value: |m, v| {
            m.first_name = v.to_string();
        },
        select_options: &[],
    },
    ColumnDef {
        key: "salutation",
        label_key: Key::Salutation,
        editable: true,
        ...
        set_value: |m, v| {
            m.salutation = if v.is_empty() {
                None
            } else {
                SalutationTO::from_str(v)
            };
        },
        select_options: SALUTATION_OPTIONS,
    },
    ColumnDef {
        key: "title",
        label_key: Key::Title,
    ...
    ColumnDef {
        key: "shares_at_joining",
        label_key: Key::SharesAtJoining,
        editable: true,
        input_type: InputType::Number,
    ...
    ColumnDef {
        key: "bank_account",
        label_key: Key::BankAccount,
        editable: true,
        input_type: InputType::Text,
        render: render_bank_account,
        get_value: |m| opt_str(&m.bank_account),
        set_value: |m, v| {
            m.bank_account = set_opt(v);
        },
        select_options: &[],
    },
    ColumnDef {
        key: "comment",
        label_key: Key::Comment,
        editable: true,
        input_type: InputType::Text,
    ...
    ColumnDef {
        key: "status",
        label_key: Key::MemberStatus,
        editable: true,
        input_type: InputType::Select,
        render: render_status,
        get_value: |m| m.status.as_str().to_string(),
        set_value: |m, v| {
            if let Some(s) = MemberStatusTO::from_str(v) {
                m.status = s;
            }
        },
