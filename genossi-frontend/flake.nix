{
  description = "Genossi Frontend - Inventory Management System";
      ...
      let
        ...
        frontend-build = pkgs.rustPlatform.buildRustPackage rec {
          ...
          buildPhase = ''
            ...
            if [ -f "target/wasm32-unknown-unknown/release/genossi-frontend.wasm" ]; then
              cp target/wasm32-unknown-unknown/release/genossi-frontend.wasm dist/
              
              # Generate JS bindings with wasm-bindgen
              wasm-bindgen --out-dir dist --target web target/wasm32-unknown-unknown/release/genossi-frontend.wasm
...
EOF
        ...
        in {
          packages.default = pkgs.runCommand "genossi-frontend" {
            allowReferences = [ ];
          } ''
            mkdir -p $out
