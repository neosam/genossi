...
#[component]
pub fn BarcodeScanner(on_scan: EventHandler<ScanResult>, on_close: EventHandler<()>) -> Element {
    ...
    let start_scanner = use_coroutine({
        ...
        move |_: UnboundedReceiver<()>| async move {
            ...
            add_debug(format!(
                "Current location: {}",
                window.location().href().unwrap_or("unknown".to_string())
            ));
            ...
            if js_sys::Reflect::has(&navigator, &JsValue::from_str("mediaDevices")).unwrap_or(false)
            {
                ...
                let media_devices =
                    js_sys::Reflect::get(&navigator, &JsValue::from_str("mediaDevices")).unwrap();
                if js_sys::Reflect::has(&media_devices, &JsValue::from_str("getUserMedia"))
                    .unwrap_or(false)
                {
                    add_debug("getUserMedia is available".to_string());
                } else {
                    add_debug(
                        "ERROR: getUserMedia is not available - WebRTC not supported".to_string(),
                    );
                    error_message.set(Some(
                        "Camera access not supported on this device/browser".to_string(),
                    ));
                    return;
                }
            } else {
                error_message.set(Some(
                    "MediaDevices API is not available in this browser".to_string(),
                ));
            ...
            let quagga_exists =
                js_sys::Reflect::has(&window, &JsValue::from_str("Quagga")).unwrap_or(false);
            ...
            if document
                .get_element_by_id(&scanner_container_id.read())
                .is_none()
            {
...
async fn load_script_from_url(url: &str, timeout_ms: u32) -> Result<(), String> {
    ...
    let script = document
        ...
        .map_err(|e| format!("Failed to create script element: {:?}", e))?;
    let script = script
        .dyn_into::<web_sys::HtmlScriptElement>()
        .map_err(|e| format!("Failed to cast to script element: {:?}", e))?;
    ...
    match result {
        ...
        futures::future::Either::Left((Err(_), _)) => {
            Err("Script loading was cancelled".to_string())
        }
        futures::future::Either::Right((_, _)) => {
            Err(format!("Script loading timed out after {}ms", timeout_ms))
        }
    }
}
...
async fn load_webrtc_adapter() -> Result<(), String> {
    ...
    for (i, url) in adapter_urls.iter().enumerate() {
        ...
        match load_script_from_url(url, 15000).await {
            Ok(_) => {
                web_sys::console::log_1(&JsValue::from_str(&format!(
                    "WebRTC adapter loaded from CDN #{}",
                    i + 1
                )));
                return Ok(());
            }
...
async fn init_quagga_with_fallbacks(container_id: &str) -> Result<(), String> {
    ...
    for (i, config) in configurations.iter().enumerate() {
        web_sys::console::log_1(&JsValue::from_str(&format!(
            "Trying camera configuration #{}",
            i + 1
        )));

        match wasm_bindgen_futures::JsFuture::from(init_quagga_js_with_config(&config.to_string()))
            .await
        {
            Ok(_) => {
                web_sys::console::log_1(&JsValue::from_str(&format!(
                    "Camera configuration #{} succeeded",
                    i + 1
                )));
                return Ok(());
            }
            Err(e) => {
                ...
                if i == configurations.len() - 1 {
                    // Last attempt failed
                    return Err(format!(
                        "All camera configurations failed. Last error: {:?}",
                        e
                    ));
                }
