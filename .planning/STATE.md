...
last_activity_desc: Quick 260908-cjo complete
progress:
  total_phases: 4
  completed_phases: 4
  total_plans: 12
...
**Current Focus:** Phase 32 — Frontend Compose-Dialog
...
Last activity: 2026-09-08 — Completed quick task 260908-cjo: Backfill rekonstruiert abgeschlossene Repayment-Phasen (historischer, status-agnostischer Resolver nur im Backfill; Live-Versandpfad unveraendert; `genossi_mail` ohne Diff)
...
### Decisions (Phase 30 Plan 01)

- D-11/D-13: `genossi_service::euro::format_eur_de(cents: i64) -> String` ist der einzige kanonische Euro-Formatter der Domäne — Tausenderpunkt, Dezimalkomma, ASCII-Space + € (U+20AC). Vorzeichen wird auf dem Betrag gebildet (Cents nie negativ), Tausendergruppierung manuell (3er von rechts), kein externes Crate ("add nothing").
- Magnitude via `(cents as i128).unsigned_abs()` statt `cents.abs()` → `i64::MIN` panic-frei (T-30-01-02 mitigiert).
- D-12: `send_confirmation_mail` (application.rs) routet den Betrag-String über `format_eur_de(total_cents)`; naive Inline-`format!("{},{:02} €")`-Zeilen entfernt. Kein e2e pinnte den Betrag-String (grep: 0 asserts).
- Test-Setup-Detail (Deviation Rule 3): `genossi_service` benötigt beim isolierten `-p`-Test das `--features utoipa`-Flag (pre-existing unbedingte `utoipa::ToSchema`-Nutzung in `auth_types.rs`). Nachfolgende Pläne/Verifier: `nix develop --command cargo test -p genossi_service --features utoipa`.
...
| Phase 15 P01 | 14min | 3 tasks | 4 files |
...
| Phase 28 P01 | 13min | 2 tasks | 2 files |
| Phase 28 P02 | 21min | 3 tasks | 6 files |
| Phase 28 P03 | 8min | 3 tasks | 2 files |
| Phase 28 P04 | 14 min | 3 tasks | 5 files |
| Phase 29 P01 | 18 min | 3 tasks | 10 files |
...
| Phase 31 P02 | 11min | 3 tasks | 3 files |
| Phase 31 P03 | 10min | 3 tasks | 3 files |
| Phase 32 P01 | 20min | 2 tasks | 4 files |
| Phase 32 P02 | 10min | 3 tasks | 4 files |
| Phase 32 P04 | 5min | 3 tasks | 5 files |
...
| Plan 20-01: `digest_state` ist eine eigene KV-Tabelle (key, value), bewusst NICHT der Config-KV-Store (D-03). Upsert via `INSERT ... ON CONFLICT(key) DO UPDATE` spiegelt `ConfigDaoSqlite`. `DigestStateDao` trägt `#[automock]` → `MockDigestStateDao` für Plan-02-Worker-Tests. `time::Date` als typsicherer Datums-Typ (TEXT 'YYYY-MM-DD' via runtime `time::format_description::parse`). | Phase 20, Plan 01 |
...
- Zweiter vorbestehender e2e-Fehlschlag: preview_body_html_round_trips_to_response (e2e_tests.rs:14961:5, 'Hallo **Max**' statt 'Hallo Max'). Ursache Quick 260718-html-to-plain-derivation. Per HEAD~1-Ruecksetzer als Nicht-Regression bewiesen; nicht gefixt (Plan 28-01 verbietet Aenderung bestehender Tests). Siehe deferred-items.md.
...
| 260607-vij | Reply-Form vorbefüllt Originalmail als Quote-Block (Outlook-Style): drei neue Props `original_body`/`original_from`/`original_date` an `InboxReplyForm`; sofort-synchrones Pre-Fill mit `> `-prefixiertem Body und deutschem `Am {date} schrieb {from}:`-Header; Signatur-Footer bleibt zwischen Eingabezone und Quote; Template-Auswahl zerstört Quote nicht mehr. Helper `build_original_quote` + `compose_initial_body` als pure functions mit 8 Unit-Tests. Frontend-only, kein Backend-Change, kein HTML-Body-Pfad. | 2026-06-07 | bfaad01 | [260607-vij-reply-form-originalmail-unten-als-quote-](./quick/260607-vij-reply-form-originalmail-unten-als-quote-/) |
| 260614-9zf | Mail-Versand persistiert gerenderten Betreff + Text pro Empfänger: neue nullable Spalten `rendered_subject`/`rendered_body` auf `mail_recipients` (Migration), `MailRecipient`-Entity + DAO INSERT/SELECT/UPDATE, Worker schreibt rendered_* vor dem Status-Update (sent + failed Pfad, Template- und Nicht-Template-Mails), `MailRecipientTO` (Backend+Frontend) exponiert beide Felder, neue wiederverwendbare Dioxus-Komponente `MailRecipientRenderedContent` zeigt sie in beiden Recipient-Tabellen (Component-First, i18n de+en). MailRecipient nicht auditiert → keine Audit-Macros. 2 neue DAO-Roundtrip-Tests + Frontend serde-default-Tests. | 2026-06-14 | b073642 | [260614-9zf-mail-versand-gerenderten-betreff-und-tex](./quick/260614-9zf-mail-versand-gerenderten-betreff-und-tex/) |
| 260614-b1t | Backfill für gerenderten Mail-Inhalt bestehender Empfänger + Kennzeichnung. Migration `20260614010000`: Spalte `rendered_reconstructed INTEGER NOT NULL DEFAULT 0` auf `mail_recipients`. Inline-Render-Logik aus `worker.rs` in geteilte Funktion `render.rs::resolve_rendered_content` extrahiert (DRY, von Worker + Backfill genutzt). Neuer idempotenter One-Shot-Backfill `backfill.rs::run_rendered_backfill`, in `genossi_bin` via `start_rendered_backfill_worker()` nach `sqlx::migrate!` in `main.rs` gewired; befüllt ALLE Zeilen mit NULL rendered_* (auch failed) und setzt `rendered_reconstructed=true`; überspringt fehlende Member/Render-Fehler (bleibt NULL → Retry beim nächsten Start). Worker setzt beim Live-Versand explizit `false`. `MailRecipientTO` (Backend+Frontend) trägt das Flag; Frontend zeigt amber Badge „Nachträglich rekonstruiert" (i18n de+en). 190 Tests grün (Render-Extraktion, DAO-Roundtrip, find_recipients_without_rendered, Backfill fill/skip/idempotent). | 2026-06-14 | 4ef4768 | [260614-b1t-backfill-rendered-subject-rendered-body-](./quick/260614-b1t-backfill-rendered-subject-rendered-body-/) |
| 260614-ckn | Job-Liste vom Mail-Versand auf separate Seite auslagern. Job-Listen-UI als wiederverwendbare Komponente `MailJobsList` nach `src/component/` extrahiert (Component-First, eigener State, Expand/Retry/Recipients/NoRepaymentLetterAction/RenderedContent + Helper-Unit-Tests). Neue Seite `MailJobsPage` unter `/mail/jobs` (Route vor `/mail/jobs/:id`) + Nav-Eintrag in Kommunikation-Gruppe (vorhandener ungenutzter i18n-Key `MailHistory` wiederverwendet, kein neuer Key). Mail-Versand-Seite (`mail_page.rs`) komplett von Job-Liste befreit (−250 Zeilen), durch Link zur neuen Seite ersetzt; verwaiste Signale/Imports/`reload_jobs()`-Aufruf bereinigt, Erfolgsmeldung bleibt; `MailJobDetail` nutzt jetzt `pub(crate)` Helper aus der Komponente (DRY). 2 Helper-Tests grün, `cargo check`/`build` sauber. | 2026-06-14 | 6721aae | [260614-ckn-job-liste-vom-mail-versand-auf-separate-](./quick/260614-ckn-job-liste-vom-mail-versand-auf-separate-/) |
| 260608-jb1 | Bug-Fix: gekündigte Mitglieder werden in Repayment-Periode eingefügt — `cancel_membership` und `transfer_shares` Voll-Übertrag erzeugen jetzt symmetrisch zu `partial_repayment` einen `RepaymentEntry` (Phase-Resolve-or-Create + Skip-bei-Duplikat + 409 bei Closed-Phase). 5 neue E2E-Tests (4× cancel, 1× transfer-full), 3 Service-Unit-Tests um Mock-Expectations erweitert. Neue Process-Strings `member-adjust.cancel.repayment` + `member-adjust.transfer-full.repayment` (disambiguiert vom bestehenden `MemberAction::Austritt`-Process). Pre-existing failure in `test_mail_preview_repayment_no_entries_does_not_default_to_one` verifiziert (out-of-scope, hängt an Quick-260602-c19-Familie). Debug-Session `kuendigung-repayment-missing` resolved. | 2026-06-08 | e856ff4b | [260608-jb1-bug-fix-gekuendigte-mitglieder-in-repaym](./quick/260608-jb1-bug-fix-gekuendigte-mitglieder-in-repaym/) |
| 260715-apr | Jahres-Export-Bug fix: Mitglieder mit `exit_date == stichtag` (typisch 31.12.) wurden aus `mitgliederliste-YYYY.csv` ausgeschlossen. SQL-Vergleich in `genossi_dao_impl_sqlite/src/backup.rs:100` von `exit_date > ?` auf `>=` geändert (Semantik „am Austrittstag noch Mitglied"). 3 Regressionstests mit hand-gerolltem in-memory SQLite-Schema (Boundary at-stichtag, before-stichtag, no-exit-date). Follow-up gemerkt: `all_actions()`/`all_communications()` filtern weder `m.deleted` noch `FehlerhaftErfasst`. | 2026-07-15 | 2dc736be | [260715-apr-exit-date-boundary-fix](./quick/260715-apr-exit-date-boundary-fix/) |
| 260625-e14 | Genosse als postalisch nicht erreichbar markieren — neues erweiterbares `PostalStatus`-Enum (Varianten `Erreichbar`/`Unzustellbar`, Default `Erreichbar`) auf Member durch alle Backend-Layer (DAO-Entity + SQLite create/read/update-Mapping, Migration `20260625000000_add_postal_status_to_member.sql` als `TEXT DEFAULT 'Erreichbar'`, `Auditable::audit_fields()` mit `postal_status` als letztem Eintrag → Statuswechsel via bestehendes `audited_update!` landet automatisch in der Hashchain) + REST-Types `MemberTO`/OpenAPI + Frontend-`PostalStatusTO` + wiederverwendbarer `MemberPostalStatusSelect`-Component (Component-First) auf der Member-Detail-Seite + i18n (de/en). Versand-Flow-Integration (RepaymentLetter/Bulk-Mail/GV) bewusst ausgeklammert → separater Folge-Task; Richtung: warnen/kennzeichnen statt hart filtern. Neue Tests: PostalStatus-Roundtrip, audit_fields-Order, Member↔Entity- und MemberTO-Roundtrips, Frontend rest-types-Roundtrip. Backend `cargo build --all-targets` grün, e2e 298 passed / 1 pre-existing out-of-scope failure. | 2026-06-25 | (jj) | [260625-e14-genosse-als-postalisch-nicht-erreichbar-](./quick/260625-e14-genosse-als-postalisch-nicht-erreichbar-/) |
| 260718-h2t | Plain-Text-Alternative aus HTML ableiten — Bug C: Frontend-supplied `body` bei WYSIWYG-Input ist nur `element.innerText()` und verliert Bullets/Nummern/Titel-Marker. Backend leitet jetzt in `render.rs::resolve_rendered_content` `body` aus dem gerenderten `body_html` ab via neuem `plain_from_html`-Helper (`html2text::from_read(..., 78)`), wenn `body_html: Some(...)`. `<ul>` → `* Foo`, `<ol>` → `1. Eins`, `<h1>` → `# Titel`, `<blockquote>` → `> Zitat`, `<b>` → `**bold**`. Backward-Compat: `body_html: None` unverändert (v1.4-Plaintext-Mails byte-identisch). HTML-02 bleibt in `send.rs::build_message` intakt — dort ist `body`-Argument weiterhin raw plain; nur der Renderer davor entscheidet, was er als `body` liefert (dokumentiert im Docstring des Helpers). Neue Dep `html2text = "0.17"` (server-side only, kein WASM-Impact). 6 neue Unit-Tests + 2 bestehende Tests um body-Assertion erweitert; 261/0 lib-Tests grün. E2E-Test konnte nicht laufen (Disk 100 % voll, 877 MB frei bei 53 GB target/ — separater Follow-up, kein Regressions-Indikator). Supersedes bewusst Phase-23-Scope-Minimierungs-Entscheidung „keine Text-aus-HTML-Ableitung"; siehe PLAN.md/SUMMARY.md-Rationale. | 2026-07-18 | (jj) | [20260718-html-to-plain-derivation](./quick/20260718-html-to-plain-derivation/) |
| 260718-css | WYSIWYG-Editor + TemplatePreview CSS-Fix — Tailwind's Preflight plättete h1-h6/ul/ol/blockquote in beiden Containern zu Plain-Text, weil das referenzierte `prose prose-sm max-w-none` no-op ist (Typography-Plugin nicht installiert, `plugins: []`, 0 `.prose`-Selektoren im gebauten CSS). Fix (User-Entscheidung Option b, WYSIWYG-Treue zum Mailclient statt npm-Overhead): Neue Custom-Klasse `.mail-html-render` in `input.css` mit Browser-Default-artigem Styling für h1-h6/p/ul/ol/li/blockquote/a/b/strong/i/em/u/s; Editor + Preview switchen von `prose prose-sm max-w-none` auf `mail-html-render`; `safelist` erweitert; `assets/tailwind.css` neu gebaut via Nix-Store-`tailwindcss_3-3.4.17` (17 `.mail-html-render`-Selektoren). Neue Grep-Gate-Tests (3, analog Phase 26 EDIT-09) sperren die Rückkehr von `prose ` mit Meta-Test-Schutz gegen Self-Match; Doc-Comment-Bug fanden sich selbst im ersten Test-Run. 8 grep-gate-Tests grün. | 2026-07-18 | (jj) | [20260718-wysiwyg-editor-preview-css-fix](./quick/20260718-wysiwyg-editor-preview-css-fix/) |
| 260718-wof | WYSIWYG-Toolbar Selection-Preserve — alle 13 Toolbar-Buttons in `wysiwyg_toolbar.rs` bekommen `onmousedown: |evt| evt.prevent_default()`. Root Cause: `mousedown` → contenteditable blur → Selection.type = "None" → `execCommand("formatBlock"/"insertUnorderedList")` läuft ohne Range und wirkt sichtbar nicht (Inline-Commands Bold/Italic/Underline/Strike toggeln State-Flag und tolerierten die Regression). Neuer inline `mod grep_gate_tests` mit `every_button_has_onmousedown_prevent_default` + Meta-Test `production_region_excludes_test_module` (analog Phase 26 EDIT-09 Pattern, self-reference-hazard-safe via `production_region()`-Slice + `format!`-assembled Needles). Negativbeweis: sed-Entfernung eines onmousedown → Test failt mit korrekter Diagnose; nach Restore 5/5 grün. Nur `wysiwyg_toolbar.rs` angefasst (Link-Dialog nicht betroffen, kein Contenteditable-Kontext). | 2026-07-18 | (jj) | [20260718-wysiwyg-toolbar-onmousedown-fix](./quick/20260718-wysiwyg-toolbar-onmousedown-fix/) |
| 260724-8p1 | Fix Phase-27 Bild-Preview-404 (aus UAT): `image_insert_html()` baute die Preview-`<img src>` relativ (`/api/mail/assets/{id}/bytes`) und umging `config.backend`. Auf beta (`config.backend`=`.../api`, echte API-Calls laufen via Reverse-Proxy über `.../api/api/...`) landete die relative URL auf einer nicht existierenden Backend-Route → 404, Bild erschien nicht im Editor. Fix: `image_insert_html(backend, id)` baut `src` jetzt `{backend}/api/mail/assets/{id}/bytes` — identisch zur bewiesenermaßen funktionierenden API-Basis; beide Caller (Toolbar-Button + Editor-Drop) übergeben `&config.backend`. Sicher, weil Sanitizer die `src` beim Speichern strippt (nur `data-genossi-asset-id` persistiert, Versand → CID). Regression-Guard `preview_src_uses_backend_base_not_relative` + angepasster Shape-Test; `cargo test wysiwyg` 18/18 grün. Backend unverändert (e2e-Roundtrip war schon grün). Out-of-scope: Drag&Drop feuert nicht (`ondrop`, separat, vom Nutzer zurückgestellt). | 2026-07-24 | (jj) | [260724-8p1-fix-phase-27-bild-preview-404-image-inse](./quick/260724-8p1-fix-phase-27-bild-preview-404-image-inse/) |
| 260729-9ld | Hotfix OIDC-Login: Provider liefert `preferred_username` nicht mehr. `register_session` (`genossi_rest/src/session.rs`) leitet die `user_id` jetzt aus dem `sub`-Claim ab (`oidc_claims.subject()`, nach OIDC Core 1.0 in jedem ID-Token Pflicht → kein `Option`). Der alte stille Fallback auf `"NoUsername"` ist ersatzlos entfallen — er ließ den Login nie hart scheitern, sondern schleuste alle Nutzer in einen gemeinsamen, per `ensure_user_exists()` auto-registrierten Account ohne Rollen. Neue Hilfsfunktion `normalize_username(&str) -> Option<String>` (trimmt, `None` bei leer) unter `#[cfg(any(feature = "oidc", test))]`; bei `None` bricht der Handler mit 500 ab statt ein Session-Cookie zu setzen. `doc/AUTHENTICATION.md` angeglichen. 5 neue Unit-Tests. **Offen vor Rollout:** bestehende `users`-Zeilen stammen aus der `preferred_username`-Ära — weicht `sub` ab, entstehen neue rechtlose Accounts und Rollen müssen einmalig umgehängt werden (Datenschritt, nicht Teil des Patches). | 2026-07-29 | (jj) | [260729-9ld-oidc-login-sub-claim-statt-preferred-use](./quick/260729-9ld-oidc-login-sub-claim-statt-preferred-use/) |
| 260908-9ud | Backfill-/Render-Logging diagnostizierbar machen — der Startup-Backfill meldete `undefined value (in <string>:3)` ohne Variablennamen, Job- oder Member-Bezug (11 von 11 Zeilen uebersprungen, Ursache nur per SQL gegen Prod-DB ermittelbar). `TemplateError` bekommt ein `diagnosis: Option<String>`, das ausschliesslich ins Log geht — `message` bleibt byte-identisch, weil es via `mark_recipient_failed` in `mail_recipients.error` persistiert und im Frontend angezeigt wird. Die Diagnose (`kind`/`line`/`expr`/`caused_by`) laeuft ueber `minijinja::Error::range()` + `str::get()` auf den uebergebenen Template-String; `display_debug_info()` wird bewusst NICHT benutzt, weil dessen `Referenced variables:`-Block die Variablen-WERTE rendert (unmaskierte IBAN aus `bank_account`, E-Mail, `current_balance`). Konsequent dazu `set_debug(false)` explizit in `strict_env()` + `html_env()`: der minijinja-Default ist `cfg!(debug_assertions)`, haette die Werte also im Dev-/Test-Build sehr wohl ins Error-Objekt gezogen — jetzt gilt in allen Build-Profilen, dass `debug_info()` None ist und selbst `{:#}` nichts leaken kann. `range()`/`line()` funktionieren trotzdem, weil der span-Pfad (`vm/mod.rs:1087-1095`, `parser.rs:1337-1341`) ausserhalb des `cfg(debug)`-Blocks und unabhaengig vom Laufzeit-Flag laeuft. Slicing nur via `str::get`, nie `&s[range]` (Umlaute + degenerierte Syntax-Spans → sonst Panic im Fehlerpfad des Startup-Backfills). `RenderFailure` reicht `diagnosis` durch; die drei bisher stillen Repayment-Merge-Zweige in `render.rs` (Phase nicht gefunden / `EntityNotFound` → Kontext bleibt unmerged / erfolgreicher Merge) loggen jetzt mit `phase_id`+`member_id`. Backfill-Warnungen tragen strukturierte Felder (`recipient_id`, `mail_job_id`, `member_id`, `application_id`, `template_id`, `repayment_phase_id`, `diagnosis`). Reine Observability — identische Fuell-/Skip-Entscheidungen, kein Non-Strict-Fallback (2 Regressionstests). 14 neue Tests, u.a. `render_error_diagnosis_names_the_undefined_variable` (Wertbeweis), `diagnosis_works_without_runtime_debug_flag` (Release-Beweis) und zwei DSGVO-Gates (weder IBAN noch Nachname in Diagnose/Log). 325 genossi_mail-Tests + Workspace-Suite gruen. Nebenbefund: die in STATE.md als pre-existing rot gefuehrten `/api/mail/preview`-e2e-Tests sind gruen (8 passed) — Notiz veraltet. | 2026-09-08 | e00bda0 | [260908-9ud-backfill-render-logging-diagnostizierbar](./quick/260908-9ud-backfill-render-logging-diagnostizierbar/) |
| 260908-cjo | Backfill rekonstruiert abgeschlossene Repayment-Phasen — Folge-Quick zu 260908-9ud, dessen Diagnose `expr=fiscal_year` als Ursache der 11 uebersprungenen Alt-Mails benannte. Die RepaymentPhase existiert, ist aber ABGESCHLOSSEN: alle Eintraege stehen auf `PaidOut`, `aggregate_for_member` filtert genau die weg (`Open`/`Contacted`-Praedikat), liefert `None` -> `EntityNotFound` -> `render.rs` laesst den Kontext bewusst unmerged -> strict-Render bricht an `fiscal_year` ab. Die Daten sind vollstaendig da, nur der Statusfilter blendet sie aus. Fix ueber eine ZWEITE Trait-Implementierung statt eines Modus-Parameters: `HistoricRepaymentContextResolverImpl` aggregiert STATUS-AGNOSTISCH (alle nicht-geloeschten Eintraege des Mitglieds in der Phase) und wird ausschliesslich als lokale Variable in `start_rendered_backfill_worker` konstruiert — nie in ein `RestStateImpl`-Feld geschrieben. Bewusst nicht "nur PaidOut zusaetzlich, wenn kein Open existiert": diese Regel waere unstetig (1 Open + 5 PaidOut ergibt 1, 0 Open + 5 PaidOut ergibt 5) und lieferte bei Mischfaellen zu kleine Betraege. `start_mail_worker` und der `RepaymentLetterService` behalten den strikten Resolver — der Open/Contacted-Filter ist beim Live-Versand fachlich richtig, weil er die Auszahlungsmail an bereits ausgezahlte Mitglieder verhindert. Weil `resolve_rendered_content` generisch ueber `RCR: RepaymentContextResolver` ist und daraus nur `aggregate` ruft, liegt die gesamte abweichende Semantik hinter genau einer Trait-Methode: `genossi_mail` bekommt KEINE Zeile Diff (verifiziert, 325/325 gruen). Geteilt statt dupliziert: `context_from_relevant` (Euro-Formatierung) und `load_phase_and_entries` (DAO-Ladelogik). Divergenz-Log (`warn!` mit `phase_id`/`member_id`/`entry_count`/`stale_status_count`) fuehrt bewusst KEINE Geldwerte und keine Anteilszahlen (D-06, Konvention aus 260908-9ud). Kennzeichnung bleibt das bestehende `rendered_reconstructed`-Badge, bekommt aber einen Tooltip, der sagt dass die Zahlen aus dem HEUTIGEN Datenstand stammen — kein zweites DB-Feld, keine Migration. Bewusst NICHT geloest: `fiscal_year` aus dem Aggregat herausloesen (wuerde den Live-Pfad aendern), Zeitschranke `entry.created <= job.created` (geprueft, verworfen — der Statuswechsel-Zeitpunkt wird nirgends gespeichert). 14 neue Tests, u.a. die beiden Pflichttests (Backfill rendert die abgeschlossene Phase; Live-Pfad scheitert an derselben Lage unveraendert mit `fiscal_year`-Diagnose) und `strict_trait_aggregate_still_errs_for_paid_out_only` als Aggregations-Regressionsgate. Workspace-Suite gruen (exit 0). | 2026-09-08 | 13e67a3, ee56e04, 8a9953b | [260908-cjo-backfill-rekonstruiert-abgeschlossene-re](./quick/260908-cjo-backfill-rekonstruiert-abgeschlossene-re/) |
...
**Last session:** 2026-08-20T23:16:33.061Z
...
- [Phase ?]: Phase 28-01: Sanitize-vor-Render im preview_mail-Handler (D-02) — ammonia zuerst, minijinja danach; spiegelt die Produktion (Store-Boundary vor Send-Worker).
- [Phase ?]: Phase 28-01: sanitize_body_html_opt wiederverwendet statt neuer Option-Verzweigung (None in => None out, kein Some-Leerstring-Sentinel).
- [Phase ?]: Phase 28-01: Nebeneffekt D-01/D-16 — bestehende TemplatePreview rendert ab jetzt die sanitisierte Fassung; bis 28-02 bleibt img ohne src leer. Relevant fuer UAT 28-05.
- [Phase ?]: Phase 28-02: Baseline-Stylesheet der Vorschau bewusst nicht als Kopie von .mail-html-render (D-10): identische Optik würde den Phasenzweck unterlaufen
- [Phase ?]: Phase 28-02: Kein Escaping des Vorschau-Dokuments (D-09): Dioxus setzt Attribute per setAttribute ohne HTML-Quelltext-Parsing
- [Phase ?]: Phase 28-02: Uuid::parse_str vor der Asset-URL-Interpolation als eigenständige Schicht, unabhängig von ammonia und iframe-Sandbox (T-28-07)
- [Phase ?]: Phase 28-03: Off-Screen-Positionierung statt Rendering-Unterdrueckung fuer den contenteditable-Knoten (T-28-12): display:none laesst inner_text() auf textContent zurueckfallen und wuerde den text/plain-Teil der Mail umbruchlos ueberschreiben
- [Phase ?]: Phase 28-03: Grep-Gate-Needle zielt auf die RSX-Aufrufform editor_container_style(*mode.read()) statt auf die Definition, sonst beweist der Gate die Verdrahtung nicht
- [Phase ?]: Phase 28-03: Modus-State lebt INNERHALB des WysiwygEditor (D-13); der key-Bump-Remount setzt ihn bewusst auf Edit zurueck (T-28-18 accept)
- [Phase 28]: Pflicht-Prop statt #[props(default)] fuer Signal-Props — Ein Default legt bei jedem Eltern-Render ein neues Signal im Eltern-Scope an: Zustand geht verloren, Signale akkumulieren (T-28-23). Zulaessig, weil alle vier Aufrufstellen im selben Plan mitgezogen wurden.
...
- [Phase ?]: 31-02: send_mail returns Result<(), ServiceError> with CR-02 ordering + no silent () — enqueue Err propagates as 500
- [Phase ?]: 31-02: preview_mail reuses the shared render_application_content kernel (D-06) — preview == worker output
- [Phase ?]: 31-02: last_sent_at = MAX(entry.date) over get_application_communications (OQ1 Option A, no new DAO query)
- [Phase ?]: 31-03: SendApplicationMailRequest hat KEIN Empfänger-Feld (D-13) — Empfänger ist immer application.email, serverseitig aufgelöst; kein Massenversand, kein Tracking.
- [Phase ?]: 31-03: communications-Handler gatet über application_service().get() ZUERST (Pitfall 3, T-31-01), nicht über den ungegateten communication_rest-Handler.
- [Phase ?]: 31-03: ApplicationTO.last_sent_at defaultet auf None in From<&Application>; nur get_application befüllt es serverseitig (APHIST-02) — kein Wire-Bruch anderer Endpoints.
- [Phase ?]: 32-01: rendered_body/rendered_html_body additiv durch CommunicationEntry-Kette (Backend + Frontend rest-types), kein Re-Render, keine Migration
- [Phase ?]: 32-02: Application-Mail-Request-Typen als lokale Frontend-Structs gespiegelt (Landmine 1); MailTemplateTO.template_type + reine filter/last-sent-Helfer
- [Phase ?]: 32-03: Aufgeloeste Compose-Vorschau ueber MailPreviewFrame (nicht member-scoped TemplatePreview), gespeist aus preview_application_mail
- [Phase ?]: 32-03: TemplateSelector additiv um filter_type + initial_template_id erweitert (backward-kompatibel, controlled select)
