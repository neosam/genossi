...
- **HTTP-Status-Codes**: 200 (Success), 400 (Validation: shares <= 0, shares >= current_shares = Voll-Rückgabe-Block, willensbekundung_date out-of-bounds), 401 (No Auth), 403 (No Admin), 404 (Member not found), 409 (Conflict: gekündigtes Mitglied, oder keine Vorgänger-Phase + Default-Fallback wurde NICHT gewählt — siehe D-16-06).
