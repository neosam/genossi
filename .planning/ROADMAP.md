...
- [x] 30-01-PLAN.md — `format_eur_de` in `genossi_service` (Null/Negativ/Tausenderpunkt) + `send_confirmation_mail`-Retrofit (APTPL-02, D-11/D-12/D-13)
- [x] 30-02-PLAN.md — `template_type`-Spalte durch alle 8 `mail_templates`-SQL/Struct/TO/Service-Sites fädeln + „Zahlungserinnerung"-Seed (UUID …0003) (APTPL-01, APTPL-03, D-01/D-02/D-03/D-14)

**Wave 2** *(blocked on Wave 1)*

- [x] 30-03-PLAN.md — `application_to_template_context` + `dummy_application_context` + `validate_rendered`-Kern + `validate_application_template` + Create/Update-Injection + Seed-Render-Beweis (APTPL-01, APTPL-04, APTPL-03, D-04..D-10)
...
- [x] 31-02-PLAN.md — Service: `send_mail` (CR-02, echtes `Result`, single-recipient) + `preview_mail` (geteilter Kernel, D-06) + `last_sent_at` (MAX über `get_application_communications`) + Service-Unit-Tests (APMAIL-01/02, APCMP-01/02, APHIST-02)
...
- [x] 32-02-PLAN.md — Dedizierte api.rs-Funktionen + template_type-Filter + last-sent-Helfer + i18n-Keys [Wave 1]
...
- [x] 32-04-PLAN.md — application_detail Button + last-sent + Timeline-Abschnitt + Body-Panel (on_entry_click) [Wave 3]
